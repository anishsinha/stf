...
{
    public class DemandCaptureUploadProcessor : IQueueMessageProcessor
    {
        public QueueProcessType ProcessorName => QueueProcessType.DemandCaptureUpload;
        ...
        public bool Process(QueueMessageViewModel queueMessage, out List<string> errorInfo, out string queueMessageJson)
        {
            ...
            if (queueMessage.RetryCount < 2)
            {
                ...
                domain.ProcessDemandCaptureUpload(input, errorInfo);
                return true;
            }
            else
            {
