...
{
    public class InvoiceImageProcessor : IQueueMessageProcessor
    {
        public QueueProcessType ProcessorName => QueueProcessType.InvoiceImageUpload;
        ...
        public bool Process(QueueMessageViewModel queueMessage, out List<string> errorInfo, out string queueMessageJson)
        {
            ...
            var input = JsonConvert.DeserializeObject<InvoiceImageProcessorReqViewModel>(queueMessageJson);
            if (input == null)
            {
            ...
            if (queueMessage.RetryCount < 2)
            {
                ...
                invBulkDomain.ProcessBulkImages(input, errorInfo);
                return true;
            }
            else
            {
