...
{
    public class PoNumberBulkUploadProcessor : IQueueMessageProcessor
    {
        public QueueProcessType ProcessorName => QueueProcessType.PoNumberBulkUpload;
        ...
        public bool Process(QueueMessageViewModel queueMessage, out List<string> errorInfo, out string queueMessageJson)
        {
            ...
            var input = JsonConvert.DeserializeObject<PoNumberBulkUploadProcessorReqViewModel>(queueMessageJson);
            if (input == null)
            {
            ...
            {
                var poNumberBulkDomain = ContextFactory.Current.GetDomain<PoNumberBulkUploadDomain>();
                poNumberBulkDomain.ProcessBulkUploadJsonMessage(input, errorInfo);
                return true;
            }
            else
            {
