...
{
    public class CsvDryRunFee
    {
        public CsvDryRunFee(string dryRunFee)
        {
            FeeSubType = FeeSubType.NoFee;
            if (!string.IsNullOrWhiteSpace(dryRunFee))
            {
                var fields = dryRunFee.Split(' ');
                ...
                {
                    throw new FormatException("Dry Run Fee is not in proper format");
                }
            }
        }
