(function ($) {
    $.fn.pxpaginate = function (_options, _callback) {
        ...
        var defaults = {
            ...
            nextPageName: 'Next',
            lastpagename: '<i class="fa fa-fast-backward" > </i>',
            firstpagename: '<i class="fa fa-fast-forward" > </i>',
        ...
        for (var i = 0; i < (_op.totalPageCount > _op.maxBtnCount ? _op.maxBtnCount : _op.totalPageCount); i++) {
        ...
        function __calcpagenumber(currentpage, isfirst) {
            ...
            if (_op.totalPageCount <= _op.maxBtnCount) {
                start = 0;
                for (var i = 0; i < _op.totalPageCount; i++) {
                    start++;
                ...
                var _blockchange = ($(".px-btn-page[data-page='" + prev + "']", _self).length == 0 || $(".px-btn-page[data-page='" + next + "']", _self).length == 0);
                if (_blockchange) {
                    var lastpagenm = (_op.totalPageCount - 1);
                    ...
                    for (var i = 0; i < _op.maxBtnCount; i++) {
                        start++;
                        $(".px-btn-page.px-btn-" + i, _self).attr("data-page", start).html(start);
                    }

                    var blockfirst = parseInt($(".px-btn-page.px-btn-0", _self).attr("data-page"));
                    var blocklast = parseInt($(".px-btn-page.px-btn-" + (_op.maxBtnCount - 1), _self).attr("data-page"));
                    
                    var pointzero = ".px-points[data-point='0']";
                    var pointone = ".px-points[data-point='1']";
