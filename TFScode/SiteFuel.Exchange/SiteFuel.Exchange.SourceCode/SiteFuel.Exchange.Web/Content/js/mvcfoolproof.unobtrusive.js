...
(function () {
    ...
    jQuery.validator.addMethod("requiredif", function (value, element, params) {
        ...
        var dependentTestValue = params["dependentvalue"].toLowerCase();
        var operator = params["operator"];
        var pattern = params["pattern"];
        var dependentPropertyElement = document.getElementsByName(dependentProperty);
        var dependentValue = null;

        if (dependentPropertyElement.length > 1 && (dependentPropertyElement[0].type == 'radio' || dependentPropertyElement[0].type == 'checkbox')) {
            for (var index = 0; index != dependentPropertyElement.length; index++)
                if (dependentPropertyElement[index]["checked"]) {
                    dependentValue = dependentPropertyElement[index].value.toLowerCase();
                    break;
                }

            if (dependentTestValue == 'true' || dependentTestValue == 'false')
                dependentTestValue = (dependentTestValue == 'true');
            if (dependentValue == null)
                dependentValue = false
			else if (dependentValue == 'true' || dependentValue == 'false')
                dependentValue = (dependentValue == 'true');
        }
        else
            dependentValue = dependentPropertyElement[0].value.toLowerCase();
