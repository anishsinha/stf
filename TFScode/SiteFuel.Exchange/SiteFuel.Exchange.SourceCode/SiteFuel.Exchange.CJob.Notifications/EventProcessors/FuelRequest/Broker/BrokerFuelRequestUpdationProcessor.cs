...
{
	public class BrokerFuelRequestUpdationProcessor : BaseFuelRequestEventProcessor, IEmailProcessor
    {
		private NotificationFuelRequestCreatedViewModel viewModel;

        public EventType EventType => EventType.BrokerFuelRequestUpdated;

        public BrokerFuelRequestUpdationProcessor()
        {
        }
		...
		public override void SendDefaultSupplierEmailForEvent(NotificationEventViewModel notificationEventViewModel, List<NotificationUserViewModel> defaultRecievers)
		{
            ...
            SendBrokerFuelRequestUpdatedEmailToCompanyAdmins(viewModel, callbackUrl, (int)notificationEventViewModel.EventType);
        }
		...
		public override void SendEmail(NotificationDomain notificationDomain, NotificationEventViewModel notificationEventViewModel)
		{
            try
            {
                if (viewModel.Id > 0)
                {
                    ...
                    var callbackUrl = $"~/Supplier/FuelRequest/Details?id={viewModel.Id}";
                    var notification = notificationDomain.GetNotificationContent(EventSubType.BrokerFuelRequestUpdated_Owner, _serverUrl, callbackUrl, (int)notificationEventViewModel.EventType);
            ...
            {
                LogManager.Logger.WriteException("BrokerFuelRequestUpdationProcessor", "SendEmail", "Exception Details : ", ex);
            }
        }

        private void SendBrokerFuelRequestUpdatedEmailToCompanyAdmins(NotificationFuelRequestCreatedViewModel viewModel, string callbackUrl, int eventTypeId)
        {
            ...
            {
                var notification = notificationDomain.GetNotificationContent(EventSubType.BrokerFuelRequestUpdated_CompanyAdmin, _serverUrl, callbackUrl, eventTypeId);
