...
{
	public class BrokerFuelRequestCancellationProcessor : BaseFuelRequestEventProcessor, IEmailProcessor
    {
		private NotificationFuelRequestCreatedViewModel viewModel;

        public EventType EventType => EventType.BrokerFuelRequestCancelled;

        public BrokerFuelRequestCancellationProcessor()
        {
        }
		...
		public override void SendDefaultSupplierEmailForEvent(NotificationEventViewModel notificationEventViewModel, List<NotificationUserViewModel> defaultRecievers)
		{
            ...
            SendBrokerFuelRequestCancelledEmailToCompanyAdmins(viewModel, callbackUrl, (int)notificationEventViewModel.EventType);
        }
		...
		public override void SendEmail(NotificationDomain notificationDomain, NotificationEventViewModel notificationEventViewModel)
		{
            try
            {
                if (viewModel.Id > 0)
                {
                    ...
                    var callbackUrl = $"~/Supplier/FuelRequest/Edit?id={viewModel.Id}";
                    var notification = notificationDomain.GetNotificationContent(EventSubType.BrokerFuelRequestCancelled_Owner, _serverUrl, callbackUrl, (int)notificationEventViewModel.EventType);
            ...
            {
                LogManager.Logger.WriteException("BrokerFuelRequestCancellationProcessor", "SendEmail", "Exception Details : ", ex);
            }
        }

        private void SendBrokerFuelRequestCancelledEmailToCompanyAdmins(NotificationFuelRequestCreatedViewModel viewModel, string callbackUrl, int eventTypeId)
        {
            ...
            {
                var notification = notificationDomain.GetNotificationContent(EventSubType.BrokerFuelRequestCancelled_CompanyAdmin, _serverUrl, callbackUrl, eventTypeId);
